...
// .wrangler/tmp/bundle-B8Rv51/checked-fetch.js
...
app.get("/", (c) => {
  return c.html(`
    ...
    <head>
      ...
      <title>Music MCP - Open Source Music for AI Agents</title>
      <style>
        body { font-family: system-ui; margin: 0; padding: 2rem; background: #111; color: #fff; line-height: 1.5; }
        .container { max-width: 800px; margin: 0 auto; }
        h1 { font-size: 3rem; background: linear-gradient(90deg, #ff00cc, #3333ff); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
        input { padding: 0.8rem; font-size: 1rem; border: 1px solid #333; border-radius: 4px; background: #222; color: #fff; width: 300px;}
        button { padding: 0.8rem 1.5rem; font-size: 1rem; border: none; border-radius: 4px; background: #fff; color: #000; cursor: pointer; font-weight: bold; }
    ...
    <body>
      <div class="container">
        <h1>Music MCP</h1>
        <p>Give your AI agents the ability to semantically search open-source, royalty-free music and get precise attribution templates.</p>
        
        <div id="auth-section">
          <h3>Get an API Key</h3>
          <p>Enter your email to receive an OTP and generate your API Key.</p>
        ...
        <div id="key-section" style="display:none;">
          ...
          <p>Configure your Claude/Gemini agent with:</p>
          <pre>
"mcpServers": {
  "music-mcp": {
    "command": "npx",
    "args": ["-y", "@modelcontextprotocol/server-sse", "--url", "https://music.builditwithai.xyz/sse?key=YOUR_API_KEY"]
  }
}
          </pre>
        </div>
...
app.post("/api/auth/otp", async (c) => {
  ...
  await c.env.DB.prepare("INSERT OR REPLACE INTO otps (email, code, expires_at) VALUES (?, ?, ?)").bind(email, otp, expiresAt).run();
  if (c.env.RESEND_API_KEY) {
    await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: "Music MCP <onboarding@builditwithai.xyz>",
        to: email,
...
app.post("/api/auth/verify", async (c) => {
  ...
  const record = await c.env.DB.prepare("SELECT code, expires_at FROM otps WHERE email = ?").bind(email).first();
  if (!record || record.code !== otp || new Date(record.expires_at) < /* @__PURE__ */ new Date()) {
    return c.json({ error: "Invalid or expired OTP" }, 401);
  }
  const apiKey = "mcp_" + crypto.randomUUID().replace(/-/g, "");
  await c.env.DB.prepare("INSERT OR IGNORE INTO users (id, email, api_key) VALUES (?, ?, ?)").bind(crypto.randomUUID(), email, apiKey).run();
  const user = await c.env.DB.prepare("SELECT api_key FROM users WHERE email = ?").bind(email).first();
  await c.env.DB.prepare("DELETE FROM otps WHERE email = ?").bind(email).run();
  return c.json({ api_key: user.api_key });
});
async function logTelemetry(env, apiKey, toolName) {
  try {
    const user = await env.DB.prepare("SELECT id FROM users WHERE api_key = ?").bind(apiKey).first();
    if (user) {
  ...
  if (method === "initialize") {
    ...
    try {
      ...
      if (vecResult.matches && vecResult.matches.length > 0) {
        const trackIds = vecResult.matches.map((m) => m.metadata?.id).filter(Boolean);
        ...
        const formatted = tracks.results.map(
          (t) => `### ${t.title} by ${t.artist}
- **ID**: \`${t.id}\`
...
${formatted}` }] };
      ...
      return c.json({ jsonrpc: "2.0", id, error: { code: -32e3, message: e.message } });
    }
    ...
    const track = await c.env.DB.prepare("SELECT title, artist, attribution_template FROM tracks WHERE id = ?").bind(track_id).first();
    if (!track) {
...
var src_default = {
  ...
  async scheduled(event, env) {
    console.log("Running ingestion cron...");
    const mockFeed = [
      {
        id: "track_1",
        ...
        attribution_template: '"Local Forecast - Slower" by Kevin MacLeod (incompetech.com). Licensed under Creative Commons: By Attribution 4.0 License.',
        audio_url: "https://incompetech.com/music/royalty-free/mp3-royaltyfree/Local%20Forecast%20-%20Slower.mp3",
        tags: "elevator, waiting, calm, ambient, slow"
      },
      {
        id: "track_2",
        title: "Tech Live",
        artist: "Kevin MacLeod",
        license_type: "CC BY 4.0",
        attribution_template: '"Tech Live" by Kevin MacLeod (incompetech.com). Licensed under Creative Commons: By Attribution 4.0 License.',
        audio_url: "https://incompetech.com/music/royalty-free/mp3-royaltyfree/Tech%20Live.mp3",
        tags: "upbeat, tech, electronic, fast"
      }
    ];
    for (const track of mockFeed) {
      await env.DB.prepare(`
        INSERT OR REPLACE INTO tracks (id, title, artist, license_type, attribution_template, audio_url, tags)
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `).bind(track.id, track.title, track.artist, track.license_type, track.attribution_template, track.audio_url, track.tags).run();
      const textToEmbed = `${track.title} by ${track.artist}. Tags: ${track.tags}`;
      const aiResponse = await env.AI.run("@cf/baai/bge-base-en-v1.5", { text: [textToEmbed] });
